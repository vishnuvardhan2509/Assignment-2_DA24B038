"""
train_model.py
Generates the synthetic spam dataset and trains a TF-IDF + MultinomialNB
classifier. Saves:
    - spam_dataset.csv   (1000 labeled messages)
...
# ---------------------------------------------------------------
# 1. Generate the dataset (deterministic, from the assignment spec)
# ---------------------------------------------------------------
random.seed(42)
...
print(f"Wrote spam_dataset.csv with {len(df)} rows")
...
acc = pipe.score(df["text"], df["label"])
...
# ---------------------------------------------------------------
examples = [
    "WIN a FREE iPhone now! Click bit.ly/xyz123",
    "Hey, are we still meeting for lunch tomorrow?",
]
print("\nExample predictions:")
for e in examples:
    print(f"  {e!r} -> {pipe.predict([e])[0]}")